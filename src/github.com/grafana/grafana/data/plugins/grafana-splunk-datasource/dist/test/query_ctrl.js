...
exports.SplunkQueryCtrl = undefined;
...
var _query_def = require('./query_def');

var _query_def2 = _interopRequireDefault(_query_def);
...
function _inherits(subClass, superClass) { if (typeof superClass !== "function" && superClass !== null) { throw new TypeError("Super expression must either be null or a function, not " + typeof superClass); } subClass.prototype = Object.create(superClass && superClass.prototype, { constructor: { value: subClass, enumerable: false, writable: true, configurable: true } }); if (superClass) Object.setPrototypeOf ? Object.setPrototypeOf(subClass, superClass) : subClass.__proto__ = superClass; } // import angular from 'angular';


var DEFAULT_INDEX = 'select index',
    DEFAULT_SOURCETYPE = 'select sourcetype',
    DEFAULT_FIELD = 'select field';

var SplunkQueryCtrl = exports.SplunkQueryCtrl = function (_QueryCtrl) {
  ...
  function SplunkQueryCtrl($scope, $injector, uiSegmentSrv, templateSrv) {
    ...
    _lodash2.default.defaultsDeep(_this.target, target_defaults);
    ...
    _this.splitBySegments = _lodash2.default.map(_this.target.splitByFields, uiSegmentSrv.newSegment);
    ...
    _this.getAppsTypeAhead = _lodash2.default.bind(getAppsTypeAhead, _this);
    return _this;
  }
  ...
  _createClass(SplunkQueryCtrl, [{
    ...
    value: function getApps() {
      ...
      return getAppsPromise.then(function (apps) {
        _this4.metrics.apps = _lodash2.default.map(apps, 'name');
        return apps;
      });
    }
    ...
    value: function getMetricAggTypes() {
      return _query_def2.default.getMetricAggTypes();
    }
  }, {
    ...
    value: function getSplitBySegment() {
      ...
      return this.getFieldSegments().then(this.uiSegmentSrv.transformToSegments(false)).then(function (segments) {
        segments.splice(0, 0, _lodash2.default.cloneDeep(_this6.removeSegment));
        return segments;
      });
    }
  }, {
    ...
    value: function getWhereSegment(index) {
      ...
      if (index === 0) {
        getOptions = Promise.resolve(_query_def2.default.getWhereOptions());
      } else {
        getOptions = Promise.resolve(_query_def2.default.getWhereConditions());
      }
      ...
      return getOptions.then(this.uiSegmentSrv.transformToSegments(false)).then(function (segments) {
        if (_this7.target.whereClause.type && index === 0) {
          segments.splice(0, 0, _lodash2.default.cloneDeep(_this7.removeSegment));
        }
        return segments;
      });
    }
    ...
    value: function getSpan() {
      return Promise.resolve(_query_def2.default.getSpanOptions());
    }
    ...
    value: function getCollapsedText() {
      if (this.target.rawQuery) {
        ...
        return _query_builder.SplunkQueryBuilder.build(this.target);
      }
    }
  }, {
    ...
    value: function renderOptions() {
      ...
      this.settingsLinkText = _query_builder.SplunkQueryBuilder.renderOptions(tcOptions, binOptions, options);
    }
  }, {
    key: 'onMetricChange',
    ...
    value: function onResultFormatChange() {
      this.splitBySegments = _lodash2.default.map(this.target.splitByFields, this.uiSegmentSrv.newSegment);
    ...
    value: function splitBySegmentChanged(segment, index) {
      ...
      this.target.splitByFields = _lodash2.default.map(_lodash2.default.filter(this.splitBySegments, function (segment) {
    ...
    value: function whereSegmentChanged() {
      if (this.whereSegments[0].value === this.removeSegment.value) {
        ...
        this.whereSegments = [];
        this.whereSegments.push(this.uiSegmentSrv.newPlusButton());
      } else if (_lodash2.default.includes(['in', 'notin'], this.whereSegments[0].value)) {
      ...
      } else if (_lodash2.default.includes(['<', '>'], this.whereSegments[0].value)) {
    ...
    value: function whereTypeChanged(operatorSegment) {
      return _lodash2.default.includes(['in', 'notin'], operatorSegment.value) && !_lodash2.default.includes(['in', 'notin'], this.target.whereClause.operator) || _lodash2.default.includes(['<', '>'], operatorSegment.value) && !_lodash2.default.includes(['<', '>'], this.target.whereClause.operator);
    }
  }, {
...
function isTargetSet(target) {
  var isMetricSet = _lodash2.default.every(target.metricAggs, function (agg) {
